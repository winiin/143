...
Категории доходов: Зарплата, Стипендия, Перевод от семьи, Другой доход`;
      ...
      if (!raw || !raw.trim()) throw new Error('ИИ вернул пустой ответ. Проверьте API ключ в профиле.');
      const stripped = raw.replace(/```json|```/gi, '').trim();
      const allMatches = stripped.match(/\{[\s\S]*\}/);
      if (!allMatches) throw new Error('ИИ ответил, но не в формате JSON. Ответ: ' + stripped.slice(0, 100));
      let parsed;
      ...
      const needsKey = errMsg.includes('api-key') || errMsg.includes('API error') || errMsg.includes('auth');
      res.innerHTML = `<div style="padding:12px;background:rgba(244,63,94,.1);border:1px solid rgba(244,63,94,.25);border-radius:var(--r);font-size:13px;color:var(--red);">
        <i class="fas fa-exclamation-circle"></i> <strong>Ошибка:</strong> ${errMsg}<br>
        ${needsKey ? `<span style="font-size:11px;color:var(--tx3);margin-top:4px;display:block;">👤 Откройте профиль (иконка справа вверху) и введите Anthropic API ключ</span>` : `<span style="font-size:11px;color:var(--tx3);margin-top:4px;display:block;">Убедитесь что файл является PDF банковской выпиской</span>`}
      </div>`;
    }
  }
}

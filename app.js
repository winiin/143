...
function defaultData() {
  return {
    transactions:[
      {id:1,text:'Зарплата',amount:300000,type:'income',category:'Доход',cardId:null,date:Date.now()-864e5*5},
      {id:2,text:'Продукты',amount:9500,type:'expense',category:'Еда',cardId:null,date:Date.now()-864e5*4},
      {id:3,text:'Такси',amount:3200,type:'expense',category:'Транспорт',cardId:null,date:Date.now()-864e5*3},
      {id:4,text:'Кино',amount:4500,type:'expense',category:'Развлечения',cardId:null,date:Date.now()-864e5*2},
    ],
    goals:[
      {id:1,name:'MacBook Pro',target:500000,saved:190000,emoji:'💻'},
      {id:2,name:'Отпуск',target:200000,saved:45000,emoji:'✈️'},
    ],
    cards:[], activeGoalId:1, level:1, xp:45, rank:'Bronze',
    nextTxId:5, currency:'KZT',
    quests:[
...
function catEmoji(cat) {
  const m={Еда:'🍽',Транспорт:'🚗',Развлечения:'🎬',Жильё:'🏠',
    Инвестиции:'📈',Здоровье:'🏥',Одежда:'👗',Доход:'💰',Прочее:'📦'};
  return m[cat]||'📦';
}
...
function parseTx(text) {
  ...
  const incKW = ['зарплат','зп','доход','премия','аванс','кешбэк','возврат','пришло','пополнение'];
  let type = 'expense', category = 'Прочее';
  for (const kw of incKW) { if (lower.includes(kw)) { type='income'; category='Доход'; break; } }
  if (type==='expense') {
    const cats = {
      'Еда':         ['кофе','обед','еда','продукт','ресторан','суши','пицц','бургер','доставк','завтрак','ужин'],
      'Транспорт':   ['такси','бензин','заправк','транспорт','метро','авто','машин','uber','яндекс'],
      'Развлечения': ['кино','игр','подписк','развлеч','бар','клуб','концерт','netflix','spotify'],
      'Жильё':       ['квартир','аренд','коммун','жкх','интернет','свет','вода'],
      'Инвестиции':  ['инвест','акци','облигац','фонд','вклад','крипт'],
      'Здоровье':    ['аптек','врач','клиник','лекарств','здоров','медиц'],
      'Одежда':      ['одежд','обувь','шопинг','магазин'],
    };
...
const TAB_LABELS = {
  ...
  ai:           {icon:'fa-robot',       label:'ИИ-советник'},
...
function renderNav() {
  ...
  nav.innerHTML=Object.entries(TAB_LABELS).map(([k,{icon,label}])=>`
    <button class="nav-btn ${k===_activeTab?'active':''}" data-tab="${k}">
...
function renderHeader() {
  ...
  document.getElementById('header').innerHTML=`
    <div class="topbar-left">
      ...
      <div>
        <div class="topbar-title"><i class="fas ${tab.icon}" style="color:var(--accent2);margin-right:8px;font-size:15px;"></i>${tab.label}</div>
...
function renderFooter() {
  document.getElementById('footer').innerHTML=`
    ...
    <div style="font-size:12px;color:var(--text3);">My Way v3.0 · ${CURRENT_USER.email}</div>`;
}
...
function renderTransactionsTab() {
  ...
  document.getElementById('content').innerHTML=`
    <div class="card" style="margin-bottom:16px;">
      <div class="card-label"><i class="fas fa-plus-circle"></i> Добавить операцию</div>
      <div class="input-group" style="margin-top:10px;">
        <input type="text" id="txInput" placeholder='"Кофе 350" или "Зарплата 200000"'/>
        ${DATA.cards.length?`<select id="txCardSel" style="max-width:200px;"><option value="">Без карты</option>${cardOpts}</select>`:''}
      ...
      <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:10px;">
        ${[['☕ Кофе','Кофе 350'],['🍽 Обед','Обед 1500'],['🚗 Такси','Такси 800'],
           ['🛒 Продукты','Продукты 5000'],['💰 ЗП','Зарплата 300000'],['📈 Инвест','Инвестиции 20000']]
          .map(([l,v])=>`<button class="btn btn-outline btn-sm template" data-text="${v}">${l}</button>`).join('')}
      </div>
    </div>
    <div class="card">
      <div class="card-header">
        <div class="card-label"><i class="fas fa-list-ul"></i> Все операции (${DATA.transactions.length})</div>
        <div style="display:flex;gap:4px;">
          <button class="btn btn-outline btn-sm filter-btn active" data-f="all">Все</button>
          <button class="btn btn-outline btn-sm filter-btn" data-f="income">Доходы</button>
          <button class="btn btn-outline btn-sm filter-btn" data-f="expense">Расходы</button>
        </div>
      </div>
...
function renderFullTxList(f) {
  ...
  }).join('')||'<div style="text-align:center;padding:20px;color:var(--text3);">Нет транзакций</div>';
}
...
function renderGoalsTab() {
  document.getElementById('content').innerHTML=`
    <div class="card" style="margin-bottom:16px;">
      <div class="card-label"><i class="fas fa-plus-circle"></i> Новая цель</div>
      <div class="input-group" style="margin-top:10px;">
        <input type="text" id="goalName" placeholder="Название цели"/>
        <input type="number" id="goalTarget" placeholder="Сумма в ₸" style="max-width:150px;"/>
        <button class="btn btn-primary" id="goalAddBtn"><i class="fas fa-plus"></i> Создать</button>
      </div>
    </div>
    <div class="card" style="margin-bottom:16px;">
      <div class="card-header">
        <div class="card-label"><i class="fas fa-bullseye"></i> Мои цели</div>
        <span style="font-size:12px;color:var(--text3);">${DATA.goals.length} целей</span>
      </div>
      <div id="goalsList"></div>
    </div>
    <div class="card">
      <div class="card-label"><i class="fas fa-route"></i> Прогресс цели</div>
      <div id="roadmap" style="padding:12px;text-align:center;color:var(--text3);font-size:14px;">Выбери цель</div>
    </div>`;
...
function renderGoalsList() {
  ...
  list.innerHTML=DATA.goals.map(g=>{
    ...
    return `<div class="goal-item ${g.id===DATA.activeGoalId?'active':''}" data-id="${g.id}">
      ...
      <button class="btn btn-primary btn-sm" data-gid="${g.id}"><i class="fas fa-plus"></i></button>
    </div>`;
  }).join('')||'<div style="text-align:center;padding:20px;color:var(--text3);">Нет целей</div>';
...
function renderRoadmap() {
  ...
  if(!g){d.innerHTML='🌟 Выбери цель';return;}
  const pct=Math.min(100,Math.round(g.saved/g.target*100));
  ...
  d.innerHTML=`
    <div style="text-align:center;">
      ...
      ${pct>=100?'<div style="margin-top:12px;font-size:14px;color:var(--green);">🎉 Цель достигнута!</div>':''}
    </div>`;
}
...
function initAIChat() {
  ...
  toggle.addEventListener('click',()=>{
    ...
    if(!visible) win.style.flexDirection='column';
  });
...
async function sendAIMsg(userMsg, container) {
  ...
  const context=buildFinancialContext();
  const sysPrompt=`Ты финансовый ИИ-советник в приложении My Way. Помогаешь пользователю ${CURRENT_USER.name} управлять финансами. Контекст: ${context}. Отвечай на русском, кратко и конкретно (2-4 предложения), используй эмодзи. Давай персонализированные советы.`;

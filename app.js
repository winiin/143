...
function renderImportTab() {
  ...
  async function processPDF(file) {
    ...
    res.innerHTML = `<div style="display:flex;align-items:center;gap:10px;padding:14px;background:var(--bg3);border-radius:var(--r);font-size:13px;">
      ...
      <span style="color:var(--tx2);">Читаю PDF и извлекаю транзакции...</span>
    </div>`;
    ...
    try {
      ...
      // Parse transactions locally — no API needed
      importedTxs = parseBankStatement(fullText);
      if (!importedTxs.length) throw new Error('Транзакции не найдены. Убедитесь что это выписка банка.');

      DATA.nextId = DATA.nextId || 1;
      importedTxs = importedTxs.map((tx, i) => ({ ...tx, id: DATA.nextId + i }));
      DATA.nextId += importedTxs.length;

      // Compute summary
      const totalIncome = importedTxs.filter(t=>t.type==='income').reduce((s,t)=>s+t.amount,0);
      const totalExpense = importedTxs.filter(t=>t.type==='expense').reduce((s,t)=>s+t.amount,0);
      ...
      res.innerHTML = `<div style="display:flex;align-items:center;gap:8px;padding:10px 14px;background:rgba(16,185,129,.1);border:1px solid rgba(16,185,129,.25);border-radius:var(--r);font-size:13px;color:var(--green);">
        ...
        <strong>Найдено ${importedTxs.length} транзакций из PDF</strong>
      </div>`;
      ...
      document.getElementById('aiAnalysisResult').innerHTML = `
        <div style="padding:16px;background:linear-gradient(135deg,rgba(124,58,237,.08),rgba(6,182,212,.05));border:1px solid rgba(124,58,237,.2);border-radius:var(--r);margin-bottom:14px;">
          <div style="font-size:13px;font-weight:700;color:var(--acc2);margin-bottom:10px;"><i class="fas fa-chart-bar"></i> Анализ выписки</div>
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;">
            <div style="text-align:center;padding:10px;background:rgba(16,185,129,.08);border-radius:8px;">
              ...
              <div style="font-size:16px;font-weight:800;color:var(--green);">+${fmtAmt(totalIncome)}</div>
            </div>
            <div style="text-align:center;padding:10px;background:rgba(244,63,94,.08);border-radius:8px;">
              ...
              <div style="font-size:16px;font-weight:800;color:var(--red);">-${fmtAmt(totalExpense)}</div>
            </div>
            <div style="text-align:center;padding:10px;background:rgba(124,58,237,.08);border-radius:8px;">
              ...
              <div style="font-size:16px;font-weight:800;color:var(--acc2);">${balance>=0?'+':''}${fmtAmt(balance)}</div>
            </div>
          </div>
        </div>`;
      ...
      document.getElementById('confirmImport').addEventListener('click', () => {
        ...
        saveData(); checkAch();
        document.getElementById('importActions').innerHTML =
          `<div style="color:var(--green);font-size:13px;font-weight:700;padding:10px;">✅ Импортировано ${importedTxs.length} операций!</div>`;
        importedTxs = [];
      });
      ...
      res.innerHTML = `<div style="padding:12px;background:rgba(244,63,94,.1);border:1px solid rgba(244,63,94,.25);border-radius:var(--r);font-size:13px;color:var(--red);">
        <i class="fas fa-exclamation-circle"></i> <strong>Ошибка:</strong> ${err.message}
      </div>`;
    }
  }
} // end renderImportTab
...
function parseBankStatement(text) {
  ...
  function guessCategory(desc, type) {
    if (type === 'income') {
      if (/зарплат|salary/i.test(desc)) return 'Зарплата';
      if (/стипенди/i.test(desc)) return 'Стипендия';
      if (/перевод|family|жания|айнур|аийша|ералы|айман/i.test(desc)) return 'Перевод от семьи';
      return 'Другой доход';
    }
    if (/такси|taxi|uber|яндекс.такси/i.test(desc)) return 'Транспорт';
    if (/magnum|дикси|small|market|продукт|супермар|grocery|кх адилет/i.test(desc)) return 'Продукты';
    if (/кафе|coffee|cafe|доставка|food|resto|lanzhou|qatty|обед|ресторан/i.test(desc)) return 'Кафе/доставка';
    if (/аптек|pharmacy|здоров|больниц|клиник|medical/i.test(desc)) return 'Здоровье';
    if (/zara|h&m|sinsay|одежд|обувь|fashion|shop|магазин/i.test(desc)) return 'Одежда/обувь';
    if (/салон|beauty|nail|парикмах|косметик|cakeDecor|decor/i.test(desc)) return 'Красота/уход';
    if (/перевод|transfer/i.test(desc)) return 'Переводы';
    if (/интернет|телефон|связь|мобил|beeline|kcell|activ|netflix|spotify/i.test(desc)) return 'Связь/подписки';
    if (/учёба|учеб|курс|school|university|образован/i.test(desc)) return 'Учёба';
    if (/цветн|подарок|gift|flower/i.test(desc)) return 'Подарки';
    return 'Другое';
  }

  // Pattern 1: Kaspi/Halyk format — "28.06.26 - 3 470,00 ₸ Покупка ИП АЙДОС"
  // or "28.06.2026 - 3 470,00 ₸ Покупка ИП АЙДОС"
  const kaspiRe = /(\d{2}\.\d{2}\.\d{2,4})\s+([+-])\s+([\d\s]+[,.]\d{2})\s*[₸T]?\s*(Покупка|Пополнение|Перевод|Снятие|Зачисление)?\s*(.*)/;

  for (const line of lines) {
    const m = line.match(kaspiRe);
    if (!m) continue;
    const [, dateStr, sign, amtRaw, opType, detail] = m;
    const amount = parseFloat(amtRaw.replace(/\s/g,'').replace(',','.'));
    ...
    const date = new Date(year, parseInt(parts[1])-1, parseInt(parts[0])).getTime();

    const isIncome = sign === '+' || /пополнение|зачисление/i.test(opType||'');
    const type = isIncome ? 'income' : 'expense';
    const desc = ((opType||'') + ' ' + (detail||'')).trim() || 'Операция';
    const category = guessCategory(desc, type);

    txs.push({ text: desc, amount, type, category, date, cardId: null });
  }

  // Pattern 2: simple "DD.MM.YYYY amount description" if pattern 1 got nothing
  if (txs.length === 0) {
    const simpleRe = /(\d{2}\.\d{2}\.\d{2,4}).*?([+-]?[\d\s]{1,10}[,.]\d{2})/g;
    let m2;
    while ((m2 = simpleRe.exec(text)) !== null) {
      const amount = parseFloat(m2[2].replace(/\s/g,'').replace(',','.'));
      if (!amount || Math.abs(amount) < 1) continue;
      const parts = m2[1].split('.');
      let year = parseInt(parts[2]); if (year<100) year+=2000;
      const date = new Date(year, parseInt(parts[1])-1, parseInt(parts[0])).getTime();
      const type = amount < 0 ? 'expense' : 'income';
      txs.push({ text: 'Операция', amount: Math.abs(amount), type, category: 'Другое', date, cardId: null });
    }
  }
